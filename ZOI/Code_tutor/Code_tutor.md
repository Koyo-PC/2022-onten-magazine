...
画像作成アプリはGimp、FireAlphaca、Adobe Ps、クリスタなどいろいろありますが、何でもいいです。
どれも持っていないという人は、無料で高機能なGimpかFireAlphacaあたりがおすすめです。
16x16くらいの画像で、いい感じに描いてみましょう。
($2^n$四方の画像を使うことが多いです。)
まあ画像なんて後からいくらでも差し変えられるので適当でいいです。

### Unityの使い方

とうとうUnityの起動です。
Unity Hubを開くと、「新しいプロジェクト」というボタンがあると思います。
そこから「2D」を選び、プロジェクト名は「MarioTest」、保存先は「C:\Develop\Unity\」みたいな感じで設定して作成しましょう。
...
まずはエディタをインストールしましょう。
世の中にはいろんなエディタがありますが、とりあえず「Visual Studio Code」というものを入れておけば問題ないでしょう。
クセがなく一番使いやすいエディタなんじゃないかと思っています。
(まあ用途によってはもっといいものもありますが)
<https://code.visualstudio.com/>からダウンロードできます。

さっそく起動してみましょう。
ただ、最初は英語設定になっているので、日本語化してみましょう。
慣れてくるまでは日本語を使ったほうがいいとおもいます。
左側にアイコンが並んでいる部分があると思いますが、その中で積み木のようになっているところをクリックしてください。
出てきた検索欄に「japanese」と入力すると「Japanese Language Pack~」というやつが出てくると思うのでインストールです。
ちょっとすると右下にRestartボタンが出てくるので、それを押してエディタを再度ひらきます。
すると日本語になっているはずです。
