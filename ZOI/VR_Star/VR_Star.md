...
![ESA-Graph](./Assets/Gaia-BV_Graph/out.png)
...
<p><img src="./Assets/UML/DataExtractor/DataExtractor_Class.png" width="81.291%" /><img src="./Assets/UML/DataExtractor/DataExtractor_Activity.png" width="18.709%" /></p>
...
---
どうやらHipparcosは角をradで保存してるらしい。やめてくれ
---

### c. データの画像化

さて画像に書き込んでいくわけですが、まるで地球の地図のように、球全体を1枚の画像で表現する必要があります。
今回は安直に、縦軸を緯度、横軸を経度にとったような図を使います。
サンソン図法を横に引き伸ばして長方形になるようにした感じです。

まあ詳しいことは考えずにやっていきましょう。
テクスチャサイズは横8192, 縦4096pxでやっていきます。

コードから画像書き出すときは`byte[]`として扱うのですが、この順番について。
どうやら一番左上のピクセルから右へ進み、端についたら2行目の左端に行く感じの順番をしてそうです。
また、1pxあたりBGRAの順番で4バイトのデータを配置します。
ただ、今回は星を扱うので、あまりに暗い星は256段階で表すと0になってしまうことがあります。
そんなに暗いのなら目に映らないから大丈夫だ、と思ったかもしれませんが、例えば天の川のあたりのように、ひとつ一つの星が暗くてもいっぱいあるから見えるということもあります。
なので、処理中は画像を`double[][][]`として扱い、どんどん加算していくことにします。
それから、後で効果を発揮するのですが、全てのピクセルは半透明として扱おうと思います。
つまり、暗い星は(r,g,b,a)=(0.1, 0.1, 0.1, 1)ではなく(r,g,b,a)=(1, 1, 1, 0.1)のように表します。

というわけで書きました。コードはこちら。
<https://github.com/ZOI-dayo/VR_Star_SkyboxRender>
