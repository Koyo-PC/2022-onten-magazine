...
public class DataExtractor {
  ...
  private const HIP_PATH = @"path¥to¥Hip.csv";
  private const CROSSMATCH_PATH = @"path¥to¥CrossMatch.csv";
  ...
  public static void Main() {
    using (var outFStream = File.Open(OUT_FILE, FileMode.OpenOrCreate)) {
      ...
      string crossMatchText = File.ReadAllText(CROSSMATCH_PATH);
      foreach (ICsvLine line in CsvReader.ReadFromText(crossMatchText)) {
        crossMatch[line[1]] = line[0];
      }
      ...
      foreach (string gaiaFile in gaiaFiles) {
        ...
        using (var reader = new StreamReader(gStream)) {
          ...
          while (reader.EndOfStream == false) {
            ...
            outFStream.Write(StarData.FromGaiaProperty(strData[5], strData[7], strData[69], strData[74], strData[79]).ToByte(), 0, 24);
          }
        }
      }
      string[] hipFiles = GetHipFiles();
      foreach (string hipFile in hipFiles) {
        // https://cdsarc.cds.unistra.fr/viz-bin/cat/I/311#/browse
        using (var fStream = File.OpenRead(HIP_PATH))
        using (var gStream = new GZipStream(fStream, CompressionMode.Decompress))
        using (var reader = new StreamReader(gStream)) {
          while (reader.EndOfStream == false) {
            string line = sr.ReadLine();
            ...
            // B-V?V-B?
            // キャスト
            outFStream.Write(StarData.FromHipProperty(line[15..29], line[29..43], line[129..137], -1*line[152..159]).ToByte(), 0, 24);
          }
        }
      }
    }
  ...
  private static string[] GetHipFiles() {
    // TODO
    // StreamReader sr = new StreamReader(GAIA_PATH + "_MD5SUM.txt");
    // string md5sumContent = sr.ReadToEnd();
    // sr.Close();
...
private class StarData {
  public final double Ra;     // 赤経
  public final double Dec;    // 赤緯
  ...
  // 別関数として独立させてしまえばいい?
  public static StarData FromHipProperty(double ra, double dec, float phot_g_mean_mag, float phot_bp_mean_mag) {
    // TODO J2000.0
    return StarData(ra, dec, phot_g_mean_mag, bvcolor);
  }
  ...
  public byte[] ToByte() {
    byte[] data = byte[24];
    Buffer.BlockCopy(BitConverter.GetBytes(Ra), 0, data, 0, 8);
    Buffer.BlockCopy(BitConverter.GetBytes(Dec), 0, data, 8, 8);
    Buffer.BlockCopy(BitConverter.GetBytes(VMag), 0, data, 16, 4);
    Buffer.BlockCopy(BitConverter.GetBytes(BVColor), 0, data, 20, 4);
